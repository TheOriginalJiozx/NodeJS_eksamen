      ...
      await toast.promise(
        apiFetch('/api/protected').then(async (res) => {
          ...
          const protectedData = await res.json();
            if (!res.ok) {
            logger.warn(`Access denied for user "${username}": ${protectedData?.message}`);
            throw new Error(protectedData?.message || 'Access forbidden');
          }
          logger.info(`Access confirmed for user "${username}"`);
...
<svelte:head>
  <script>
    try {
      ...
      logger.error({ message: 'Error checking JWT in localStorage', error });
    }
  </script>
</svelte:head>
