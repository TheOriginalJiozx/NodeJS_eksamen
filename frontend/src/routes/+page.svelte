<script>
  ...
  const optionColors = {
    Red: 'bg-gradient-to-r from-red-400 via-red-500 to-red-600 text-white shadow-lg hover:brightness-110',
    Blue: 'bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 text-white shadow-lg hover:brightness-110',
    Green: 'bg-gradient-to-r from-green-400 via-green-500 to-green-600 text-white shadow-lg hover:brightness-110',
    Yellow: 'bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 text-white shadow-lg hover:brightness-110',
    Pink: 'bg-gradient-to-r from-pink-400 via-pink-500 to-pink-600 text-white shadow-lg hover:brightness-110',
    Purple: 'bg-gradient-to-r from-purple-400 via-purple-500 to-purple-600 text-white shadow-lg hover:brightness-110'
  };

  onMount(() => {
    socket = io(PUBLIC_SERVER_URL, { transports: ['polling', 'websocket'] });
    logger.info('Socket connection established');

    /** @param {Poll} data */
    socket.on('pollUpdate', (data) => {
      poll = data;
      logger.debug({ poll }, 'Poll updated');
    });
    ...
    socket.on('connect_error', (error) => {
        ...
        logger.error({ error }, 'Socket error: unknown');
      }
  ...
  function vote(option) {
    ...
    if (!isLoggedIn) {
      ...
      logger.warn('Unauthenticated user attempted to vote');
      return;
    }
    ...
    logger.info(`User "${username}" voted for "${option}"`);
  }
</script>
  ...
  <div class="flex-grow flex flex-col items-center justify-center">
    <div class="bg-white/20 backdrop-blur-lg rounded-3xl shadow-2xl p-12 w-full max-w-2xl flex flex-col items-center border border-white/30">
      ...
      <div class="p-8 max-w-xl mx-auto w-full">
        {#if poll.question}
          <h3 class="mt-4 bg-white/30 text-white font-semibold py-2 px-4 rounded-xl transition">Question of the day</h3>
